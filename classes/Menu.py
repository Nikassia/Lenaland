...
class Menu:
    def __init__(self, screen, dashboard, level, sound):
        ...
        # Загружаем изображения из меню спрайтлиста
...
# Отрисовка текущего положения курсора в меню
    def drawDot(self):
        if self.state == 0:
...
# Загузка настроек из json, установка параметров звука и музыки
    ...
    # сохранение текущих настроек в файл json
    def saveSettings(self, url):
        data = {"sound": self.music, "sfx": self.sfx}
        with open(url, "w") as outfile:
            json.dump(data, outfile)

# Отрисовка элементов меню на экране
    def drawMenu(self):
        self.drawDot()
...
# Отрисовка фона меню и различных объектов на экране
    def drawMenuBackground(self, withBanner=True):
        ...
        # Отрисовка различных объектов на фоне
        if withBanner:
...
# Отрисовка меню настроек
    def drawSettings(self):
        ...
        if self.sfx:
            # Отображение состояния звуковых эффектов
...
# отрисовка границ элемента на экране
...
# Отображение списка готовых уровней
...
# Загрузка и возврат имен списка доступных уровней
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN: # Если нажата клавиша
                if event.key == pygame.K_ESCAPE:
                    if self.inChoosingLevel or self.inSettings: # Если находимся в режиме выбора уровня или настроек
                        self.inChoosingLevel = False # Выходим из режима выбора уровня
                        self.inSettings = False
                        self.__init__(self.screen, self.dashboard, self.level, self.sound) # Инициализируем игру заново
                    else:
                        pygame.quit()
                        sys.exit()
                elif event.key == pygame.K_UP or event.key == pygame.K_k: # Если нажата клавиша ВВЕРХ или K
                    if self.inChoosingLevel:
                        # Логика для перемещения вверх по меню или уровням
                        if self.currSelectedLevel > 3:
                            self.currSelectedLevel -= 3
                            self.drawLevelChooser()
                    if self.state > 0:
                        self.state -= 1
                elif event.key == pygame.K_DOWN or event.key == pygame.K_j: # Если нажата клавиша ВНИЗ или J
                    if self.inChoosingLevel:
                        # Логика для перемещения вниз по меню или уровням
                        if self.currSelectedLevel+3 <= self.levelCount:
                            self.currSelectedLevel += 3
                            self.drawLevelChooser()
                    if self.state < 2:
                        self.state += 1
                elif event.key == pygame.K_LEFT or event.key == pygame.K_h: # Если нажата клавиша ВЛЕВО или H
                    if self.currSelectedLevel > 1:
                        self.currSelectedLevel -= 1
                        self.drawLevelChooser()
                elif event.key == pygame.K_RIGHT or event.key == pygame.K_l: # Если нажата клавиша ВПРАВО или L
                    if self.currSelectedLevel < self.levelCount:
                        self.currSelectedLevel += 1
                        self.drawLevelChooser()
                elif event.key == pygame.K_RETURN: # Если нажата клавиша ENTER
                    # Логика для обработки выбора уровня, настроек или выхода из игры
                    if self.inChoosingLevel:
                        self.inChoosingLevel = False
                    ...
                    if not self.inSettings:
                        ...
                        if self.state == 0:
                            ...
                            self.inSettings = False
        pygame.display.update() # Обновляем экран после обработки всех событий
